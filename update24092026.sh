#!/bin/bash
# ====================================================================
# SCRIPT UPDATE: VLESS + TRIAL + SPEEDTEST + AUTO-CLEAN + HOURLY BACKUP
# ====================================================================
...
echo "⏳ Memulai pembaruan sistem dan penambahan fitur..."

# 1. Install dependensi baru (Nginx untuk VLESS & Speedtest CLI)
export DEBIAN_FRONTEND=noninteractive
apt-get update -y
apt-get install -y -o Dpkg::Options::="--force-confdef" -o Dpkg::Options::="--force-confold" \
    nginx speedtest-cli python3-pip

pip3 install speedtest-cli requests --break-system-packages 2>/dev/null || pip3 install speedtest-cli requests

# 2. Konfigurasi Nginx Multiplexer (Jalur VMess & VLESS bersamaan)
...
# Restart Nginx
service nginx restart 2>/dev/null || /usr/sbin/nginx -s reload 2>/dev/null || /usr/sbin/nginx

# 3. Update Format Config Xray (Mendukung VMess port 23331 & VLESS port 23332)
cat << 'EOF' > /tmp/update_cfg.py
import json, os
...
if os.path.exists(CFG):
    ...
    # Ambil existing clients
    clients_vmess = []
    clients_vless = []
    if "inbounds" in data and len(data["inbounds"]) > 0:
        clients_vmess = data["inbounds"][0]["settings"].get("clients", [])
        for c in clients_vmess:
            clients_vless.append({"id": c["id"], "email": c["email"]})

    # Susun ulang 2 inbound: Inbound 0 (VMess 23331), Inbound 1 (VLESS 23332)
    data["inbounds"] = [
        {
            "port": 23331,
            "listen": "127.0.0.1",
...
# Konversi format tanggal users.json ke format lengkap (YYYY-MM-DD HH:MM:SS) jika belum
if os.path.exists(USERS_FILE):
    with open(USERS_FILE) as f:
        u_data = json.load(f)
    for k, v in u_data.items():
        if "exp" in v and len(v["exp"]) == 10:
            v["exp"] = v["exp"] + " 23:59:59"
    with open(USERS_FILE, "w") as f:
        json.dump(u_data, f, indent=2)
EOF
python3 /tmp/update_cfg.py 2>/dev/null
rm -f /tmp/update_cfg.py

# 4. Update Bot Daemon (Dengan Auto-Backup Tiap 1 Jam + Speedtest + Trial + Auto-Clean RAM)
...
# BACKGROUND DAEMON 1: Pengecekan Expired & Pembersihan RAM Harian
def check_expired_and_cleanup_loop(token=None, admin_id=None):
    ...
    while True:
        try:
            ...
            # Pembersih Cache RAM Harian (Tiap pergantian hari)
...
# BACKGROUND DAEMON 2: Auto-Backup Kode Base64 Tiap 1 Jam ke Telegram
def hourly_backup_loop(token, admin_id):
    while True:
        time.sleep(3600)  # Tiap 1 Jam (3600 Detik)
        try:
            if os.path.exists(CONFIG_FILE) and os.path.exists(USERS_FILE):
                ...
                msg = (
                    f"⏰ *AUTO-BACKUP RUTIN (Tiap 1 Jam)*\n"
                    f"📅 Waktu: `{now_time}`\n\n"
                    f"Salin teks Base64 ini untuk restore di VPS baru:\n\n"
                    f"`{b64_str}`"
                )
            ...
            for upd in res.get("result", []):
                ...
                if "callback_query" in upd:
                    ...
                    if data == "bot_list":
                        ...
                        send_msg(cid, "⏳ *Sedang menjalankan Speedtest...* (Tunggu sekitar 15-20 detik)")
                        def run_st():
                            try:
                ...
                # 2. Handle Dokumen Backup (Restore)
                ...
                # 3. Handle Text
                    ...
                    if state in ["create", "trial"]:
                        ...
                        try:
                            ...
                            # Daftarkan ke VMess (Inbound 0) & VLESS (Inbound 1)
...
# 5. Update Menu Terminal (/usr/local/bin/menu)
...
echo "=========================================================="
echo "🎉 UPDATE BERHASIL 100%!"
echo "🔹 Protokol VLESS + VMess: Aktif"
echo "🔹 Fitur Trial 1 Jam & 24 Jam: Siap"
echo "🔹 Speedtest Server: Siap"
echo "🔹 Pembersih RAM Otomatis: Aktif"
echo "🔹 Auto-Backup Base64 ke Telegram: Aktif Tiap 1 Jam"
echo "=========================================================="
menu
